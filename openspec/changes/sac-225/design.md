...
The production build will be static and deploy to a Direct Upload Cloudflare Pages project named `sac-225-calculator`. The repository's preview job will run the production build, then run `npx wrangler pages deploy dist --project-name=sac-225-calculator --branch=review-sac-225`. That fixed non-production branch produces the moving alias `review-sac-225.sac-225-calculator.pages.dev`; the job will capture the deployment's unique hash URL and use that immutable URL for final review. Cloudflare documents both the `--branch` preview command and the branch alias behavior: https://developers.cloudflare.com/pages/get-started/direct-upload/.

Repository maintainers own the Pages project in the Cloudflare account selected by the protected `CLOUDFLARE_ACCOUNT_ID` CI secret. A separate protected `CLOUDFLARE_API_TOKEN` has only Account / Cloudflare Pages / Edit permission in that account; neither value reaches the browser bundle or untrusted pull-request jobs. Cloudflare's Direct Upload CI guide defines those credentials and the Wrangler deployment flow: https://developers.cloudflare.com/pages/how-to/use-direct-upload-with-continuous-integration/.

The preview deployment remains publicly readable with no Cloudflare Access policy so the final reviewer can open it; preview URLs are public by default, while enabling Access restricts them: https://developers.cloudflare.com/pages/configuration/preview-deployments/. The automatic job runs only for trusted same-repository branches after checks pass. If it is skipped, lacks secrets, or fails, a maintainer reruns the same protected job with `workflow_dispatch` for the reviewed commit; as an operational fallback, the maintainer may run the identical Wrangler command against that checked-out commit with the same account and token. No URL is submitted for review until the captured immutable URL passes an HTTPS smoke test.
...
2. Selecting an operator for a left entry stores its normalized value in `leftOperand`, stores the operator, enters `awaiting_rhs`, and leaves the existing `entry` visible. In `awaiting_rhs`, calculate is a no-op and another operator only replaces the pending operator, so `8 + =` stays at `8` with `+` pending.
3. A digit in `awaiting_rhs` replaces `entry` with that digit and changes the phase to `entering` while retaining `leftOperand` and `operator`; a decimal instead sets `entry` to `0.` and makes the same phase change. Thus `phase: entering` plus a non-null pending operation means that a right operand has been entered. The same digit or decimal in `result` starts a new calculation and clears the stored operation.
4. Calculate is a no-op unless the phase is `entering` and both `leftOperand` and `operator` are non-null. Otherwise it treats the current `entry` as the right operand, evaluates exactly one binary operation, formats the result into `entry`, clears the pending operation, and enters `result`; therefore `8 + 2 =` renders `10`. Because implicit chaining is out of scope, selecting an operator while a right operand is being entered is ignored until calculate or clear.
...
Typed values and stored operands remain strings. Before an entry becomes an operand, `normalizeOperand` removes exactly one trailing decimal point, so `0.` becomes `0` and `1.` becomes `1`; it makes no other textual or numeric conversion. Operator selection stores the normalized left operand, and calculate normalizes the right operand. This preserves unfinished text while typing but guarantees that strict `big.js` receives a completed decimal string.

At calculation time, strict `big.js` instances are created from the normalized strings. Addition, subtraction, and multiplication return exact decimal results; division is the only supported operation that can require rounding. Configure a private `Big` constructor with 20 decimal places and round-half-up, reject a right operand equal to zero before dividing, then render every result with `toFixed()` with no decimal-place argument. The big.js API defines the division controls and specifies that `toFixed()` without a decimal-place argument returns normal notation regardless of the `NE` and `PE` exponent thresholds: https://mikemcl.github.io/big.js/.

This policy preserves all entered significant digits for `+`, `-`, and `*`, so `1234567890123 + 1` displays `1234567890124`. It also makes `0.1 + 0.2` exactly `0.3`. A terminating quotient is shown without unnecessary trailing zeros; a repeating quotient is rounded to at most 20 decimal places. Results always use ordinary decimal notation: `1 / 10000000` displays `0.0000001`, and `1000000000000000000000 * 1` displays the complete integer rather than exponent notation. Negative zero is normalized to `0`.
...
Vitest table-driven tests will cover all four operations, decimal entry, duplicate-decimal rejection, exact `0.1 + 0.2`, the large exact boundary `1234567890123 + 1`, ordinary-notation boundaries `1 / 10000000` and `1000000000000000000000 * 1`, repeating-division rounding, negative results, clear from partial/result/error states, operator replacement, ignored actions in error, and recovery after division by zero. Transition tests will assert `8 + =` is unchanged, `8 + 2 =` is `10`, `1. + 2 =` is `3`, and `1 + 2. =` is `3`.
...
```text
entry: input or result decimal string
leftOperand: normalized decimal string | null
operator: "+" | "-" | "*" | "/" | null
...
While `phase` is `awaiting_rhs`, `entry` retains the visible left-entry text and calculate cannot run. After the first right-hand digit or decimal, `phase` is `entering`, `entry` holds the right-entry text, and the non-null `leftOperand` and `operator` distinguish that state from left-entry input.
...
- `entry` is always renderable text. User entry contains digits and at most one decimal point; a result may additionally contain a leading minus sign but never exponent notation.
- `leftOperand` is copied only after applying the trailing-decimal normalization rule; the displayed entry string is never reconstructed while the person is still typing.
...
For `1. + 2 =`, the left display remains `1.` when operator selection normalizes and stores `1`. Entering `2` moves from `awaiting_rhs` to `entering` and replaces the display. Calculate normalizes `2`, evaluates `1 + 2`, clears the pending fields, enters `result`, and renders `3`.
...
- **Operator followed immediately by calculate** → Keep the `awaiting_rhs` state and visible left entry; do not invent or reuse a right operand.
- **Operand ends in a decimal point** → Strip the trailing point only at the operand boundary, then evaluate the completed decimal value.
...
- **Preview job skipped or deployment fails** → Rerun the protected job or execute its identical Wrangler command for the reviewed commit; keep the last successful deployment intact and withhold review until a new immutable URL passes the smoke test.
...
- **Arbitrarily long exact results can create tall displays** → Keep normal decimal notation for predictable values and wrap within the panel; tests cover horizontal containment rather than imposing an unstated precision limit.
- **Twenty decimal places rounds repeating division** → Document that division alone is rounded half-up at that boundary and lock representative repeating quotients in unit tests.
- **A public preview exposes this static calculator to anyone with the URL** → Deploy no secrets or user data, use the immutable URL only for review, and retain Pages' preview `noindex` behavior documented at https://developers.cloudflare.com/pages/configuration/preview-deployments/.
- **Direct Upload depends on protected Cloudflare credentials** → Scope the token to Pages Edit in one account, keep both credentials in protected CI secrets, and provide the same-command maintainer fallback.
...
3. Deploy `dist` to the `sac-225-calculator` Pages project with branch `review-sac-225`, capture the immutable deployment URL, and smoke-test it over HTTPS; use the protected rerun or same-command fallback if automatic deployment is unavailable.
