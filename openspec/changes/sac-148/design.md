...
The CLI owns argument parsing, stream selection, and process exit status. The coordinator owns the use-case sequence and its limits. Three injected adapters own external I/O: a Google search gateway, an article reader, and a summary generator. The two-paragraph validator is deterministic and has no network access.
...
1. The CLI joins and trims positional arguments. If the result is empty, it emits the input error and stops.
2. The coordinator sends the normalized query to the Google search gateway. A gateway failure stops the operation as `search_failed`.
3. The coordinator normalizes, deduplicates, and selects up to five eligible results in Google rank order.
4. The article reader fetches selected pages with bounded concurrency and applies URL safety, response, extraction, and text-size limits. Individual page failures are recorded only as internal diagnostics and skipped.
...
6. The summary generator sends the bounded sources to the language model. The validator accepts the result or permits one corrective request.
